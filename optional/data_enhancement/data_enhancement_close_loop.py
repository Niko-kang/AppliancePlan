#!/usr/bin/env python3
"""
闭环数据增强脚本：根据测评结果决定是否追加增强数据
逻辑：
1. 读取结果目录下最新的 merged_close_loop*.txt（或子目录递归查找）
2. 解析 "Exact Match Accuracy" 指标
3. 若低于阈值，则从 data_enhancement_close_loop/ 中每个 JSON 随机抽取一半样本
"""
import os
import sys
import json
...
EXACT_THRESHOLD = float(os.getenv("CLOSE_LOOP_ENHANCEMENT_THRESHOLD", 0.7))
SAMPLE_RATIO = 0.5
RNG_SEED = int(os.getenv("CLOSE_LOOP_ENHANCEMENT_SEED", 42))
SUMMARY_PREFIX = "merged_close_loop"
SUMMARY_PATTERN = "merged_close_loop*.txt"
SAMPLE_OUTPUT_TEMPLATE = "enhanced_close_loop_iter_{iteration}.json"
SUMMARY_OUTPUT_TEMPLATE = "enhancement_close_loop_summary_iter_{iteration}.json"


def usage():
    print(
        "用法: python data_enhancement_close_loop.py <config_dir> <new_data_dir> <result_dir> <iteration>"
    )
    print("参数说明:")
...
def resolve_iteration_dir(config_dir: Path) -> Path:
    if config_dir.name == "log":
        return config_dir.parent
    return config_dir


def find_latest_summary(result_dir: Path):
    summary_files = list(result_dir.glob(SUMMARY_PATTERN))
    if not summary_files:
        summary_files = list(result_dir.rglob(SUMMARY_PATTERN))
    if not summary_files:
        return None
...
def save_enhanced_data(samples, new_data_dir: Path, iteration: int):
    ...
    output_path = new_data_dir / SAMPLE_OUTPUT_TEMPLATE.format(iteration=iteration)
...
def main():
    ...
    enhancement_dir = iteration_dir / "data_enhancement_close_loop"

    summary_file = find_latest_summary(result_dir)
    if not summary_file:
        logger.warning("未找到 merged_close_loop*.txt，跳过数据增强")
        return
    ...
    if exact_match is None:
        logger.warning("无法解析 Exact Match Accuracy，跳过数据增强")
        return

    logger.info(f"整体 Exact Match Accuracy: {exact_match:.4f}")

    if exact_match >= EXACT_THRESHOLD:
        logger.info(f"Exact Match ≥ {EXACT_THRESHOLD:.2f}，本轮无需追加增强数据")
        return
