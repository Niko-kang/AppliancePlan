...
for idx in "${!MODEL_PATHS[@]}"; do
  ...
  echo "Running test_close_loop.py -> ${RESULT_DIR}" | tee -a "${LOG_FILE}"
  python "${EVAL_DIR}/test_close_loop.py" \
  ...
  if [ $TEST_STATUS -eq 0 ]; then
    python3 "${EVAL_DIR}/aggregate_eval_results.py" \
      --test_script "test_close_loop.py" \
      --result_dir "${RESULT_DIR}" \
      --test_data_path "${REAL_TEST_DATA_PATH}" \
      --model_name "${MODEL_PATH}" \
      --root_path "${ROOT_PATH}" \
      --eval_name "real_close_loop" \
