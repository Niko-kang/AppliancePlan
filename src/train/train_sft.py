...
def get_node_specific_env():
    env_path = os.path.expanduser('~/.joybuilder_node_env')
    ...
    with open(env_path, 'r') as f:
        for line in f:
            line = line.strip()
            # 匹配类似 export VAR=value 或 VAR=value 的行
            match = re.match(r'^(export\s)?([a-zA-Z_][a-zA-Z0-9_]*)=(.*)$', line)
            if match and not line.startswith('#'):
                var_name = match.group(2)
                var_value = match.group(3).strip('"\'')  # 去除可能的引号
...
def train():
    ...
    data_module = make_supervised_data_module(model_id=model_args.model_id,
                                              processor=processor,
                                              data_args=data_args)
    # 打印第一条训练数据（假设是监督式学习任务）
    #print("\n=== 第一条训练数据 ===")
    #print(data_module["train_dataset"][0])  # 索引0获取第一条
...
if __name__ == "__main__":
    ...
    os.environ.update(get_node_specific_env())
    nccl_ib_gid = os.environ.get('NCCL_IB_GID_INDEX')
    if not nccl_ib_gid:
        print('WARNING!!! NCCL_IB_GID_INDEX is not set')
    else:
        print(f'NCCL_IB_GID_INDEX -------- {nccl_ib_gid}\n')
